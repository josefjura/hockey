...
test.describe('Main Pages Load', () => {
  ...
  test('events page loads', async ({ page }) => {
    ...
    await expect(page.locator('main h1')).toContainText(/events/i);
  });

  test('seasons page loads', async ({ page }) => {
    await page.goto('/seasons');
    await expect(page.locator('main h1')).toContainText(/seasons/i);
  });

  test('matches page loads', async ({ page }) => {
    await page.goto('/matches');
    await expect(page.locator('main h1')).toContainText(/matches/i);
  });

  test('management page loads', async ({ page }) => {
    await page.goto('/management');
    await expect(page.locator('main h1')).toContainText(/management/i);
  });
});
