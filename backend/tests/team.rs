...
async fn test_get_team_endpoint(
    ...
    let team_id: i64 = id.parse().map_err(|_| {
        AppError::invalid_input("Invalid team ID format")
    })?;
...
async fn business_logic_create_team_validation(pool: SqlitePool) {
    ...
    let result = TeamBusinessLogic::create_team(&ctx, Some("Valid Team".to_string()), -1, None).await;
    assert!(result.is_err());
    match result.unwrap_err() {
...
async fn business_logic_logo_path_validation(pool: SqlitePool) {
    ...
    assert!(result.is_err());
    match result.unwrap_err() {
        AppError::InvalidInput { message } => assert!(message.contains("valid URL or absolute path")),
        _ => panic!("Expected InvalidInput error"),
    }
}
...
async fn service_layer_database_operations(pool: SqlitePool) {
    ...
    ).await.expect("Failed to create team");
